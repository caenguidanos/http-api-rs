...
impl IntoResponse for common::domain::Error {
    fn into_response(self) -> Response {
        let mut problem_details: libs::problemdet::ProblemDetails;
        ...
        match self {
            ...
            | Self::InvalidProductTimeStampRelation => {
                problem_details = libs::problemdet::ProblemDetails::from_400();
                problem_details.set_detail(self);
            }
            Self::InvalidPermission => {
                problem_details = libs::problemdet::ProblemDetails::from_403();
                problem_details.set_detail(self);
            }
            Self::Persistence(reason) => {
                problem_details = libs::problemdet::ProblemDetails::from_503();
                problem_details.set_detail(reason);
            }
        }

        common::infrastructure::JsonResponse::with_status(
